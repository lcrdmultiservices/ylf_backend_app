...
def verify_account(data: VerifyRequest, request: Request, db: Session = Depends(get_db), mongo_db = Depends(get_mongo_db)):
    ...
    """
    if mongo_db is None:
        # Este error es un fallo del servidor, por lo que un 503 es apropiado.
    ...
    try:
        ...
        if not otp_document:
            ...
            # Lanzamos un error 400. FastAPI lo manejará y lo enviará al cliente.
        ...
        # CORRECCIÓN: Este bloque ahora solo capturará errores inesperados.
        # Si la excepción ya es una HTTPException (como nuestro error 400),
        # la volverá a lanzar sin modificarla.
        if isinstance(e, HTTPException):
            raise e
        
        # Si es cualquier otro error, lo registramos y devolvemos un 500.
