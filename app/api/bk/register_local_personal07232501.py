...
# --- NUEVO: Importar el cliente de Mailgun y la configuración ---
...
class RegisterLocalUser(BaseModel):
    ...
    # --- NUEVO CAMPO: Se recibe el idioma desde el frontend ---
    language: str = 'en'
...
def log_consent_to_mongo(mongo_db, user_id: int, process_key: str, option_key: str, status: str, version_id: Optional[int], request: Request):
    # ... (código sin cambios)
    if mongo_db is None:
        print(f"ERROR: No se puede registrar el consentimiento '{process_key}' porque la conexión a MongoDB no está disponible.")
        return
    ...
    try:
        ...
        print(f"Successfully logged consent '{process_key}' for user {user_id} to MongoDB.")
    except Exception as e:
        print(f"ERROR: Failed to log consent '{process_key}' for user {user_id} to MongoDB. Error: {e}")
        log_error(mongo_db=mongo_db, endpoint="/register/local/personal", method="POST-MONGO-CONSENT", error=e, request_payload={"user_id": user_id, "process_key": process_key}, user_context={"client_ip": request.client.host if request.client else "N/A"})

def associate_anonymous_consent(mongo_db, user_id: int, anonymous_id: str):
    # ... (código sin cambios)
    if mongo_db is None or anonymous_id is None:
        return
    try:
        consent_log_collection = mongo_db.consent_logs
        print(f"Attempting to associate anonymous_id {anonymous_id} with user_id {user_id}")
        result = consent_log_collection.update_many(
            {"anonymous_id": anonymous_id},
            {"$set": {"user_id": user_id}}
        )
        print(f"Association result: {result.matched_count} documents matched, {result.modified_count} documents modified.")
    except Exception as e:
        print(f"ERROR: Failed to associate anonymous consent for user {user_id}. Error: {e}")
...
# --- NUEVA FUNCIÓN: Para generar y guardar el OTP ---
def generate_and_store_otp(mongo_db, user_id: int, email: str) -> str:
    """
    Genera un OTP de 6 dígitos, lo guarda en MongoDB y lo devuelve.
    """
    if mongo_db is None:
    ...
    try:
        ...
        print(f"OTP {otp_code} generado para el usuario {user_id}.")
        return otp_code
    except Exception as e:
...
def register_local_user(data: RegisterLocalUser, request: Request, db: Session = Depends(get_db), mongo_db = Depends(get_mongo_db)):
    ...
    try:
        ...
        db.rollback()
        log_error(mongo_db=mongo_db, endpoint="/register/local/personal", method="POST-MYSQL", error=e, request_payload=data.dict(), user_context={"email": data.email, "client_ip": request.client.host if request.client else "N/A"})
        raise HTTPException(status_code=500, detail="error_registration_failed")

    # --- Lógica de consentimientos y OTP ---
    ...
    # --- NUEVO: Generar OTP y enviar correo ---
    otp_code = generate_and_store_otp(mongo_db, user_id=person_id, email=data.email)
    ...
    if otp_code:
        ...
        # Si la generación de OTP falla, se debe manejar el error.
        # Por ahora, lanzamos una excepción genérica.
        raise HTTPException(status_code=500, detail="error_otp_generation_failed")

    # --- NUEVO: La respuesta ahora indica que se necesita verificación por OTP ---
