...
# Importamos la dependencia get_db y la sesión de SQLAlchemy
from app.db.database import get_db

router = APIRouter()

# --- Modelo Pydantic Actualizado ---
# Ahora incluye el ID de los términos aceptados
class RegisterLocalUser(BaseModel):
    ...
    country_iso: str
    password: str
    terms_id: int # ID de los términos que el usuario aceptó
...
def register_local_user(data: RegisterLocalUser, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Si cualquier paso falla, db.rollback() deshará todo.
        db.begin()
        ...
        # 4. Vincular persona y teléfono en `person_has_phone`
        ...
        # 5. Crear la cuenta en la tabla `accounts` con la contraseña hasheada
        ...
        # 7. Vincular usuario y cuenta en `users_has_accounts`
        ...
        # 8. Registrar la aceptación de los términos en `users_terms_acceptance`
        terms_query = text("INSERT INTO users_terms_acceptance (users_idusers, terms_and_conditions_idterms_and_conditions) VALUES (:uid, :tid)")
        db.execute(terms_query, {"uid": person_id, "tid": data.terms_id})
    ...
    # 9. Enviar correo de verificación (solo si la transacción fue exitosa)
